...
<div class="dashboard-edit flex justify-between items-center py-3">
    <h4 class="dashboard-edit-title mb-1">{{ __("Import Quiz") }}</h4>
    <x-common.top-btn :icon="'fa-bars'" :text="__('All Quizzes')" class="mb-0" href="{{ route('dashboard.quiz.list') }}" />
</div>
<form method="POST" action="{{ route('dashboard.quiz.import') }}" enctype="multipart/form-data">
    @csrf
            ...
            <div class="dashboard-edit p-10">
                ...
                <div class="eduman-select-field mb-8">
                    <h5 class="text-[15px] text-heading font-semibold mb-3">
                        {{ __("Import Questions from CSV file") }}
                    </h5>
                    <div class="custom-file">
                        <input type="file" name="questions_file" class="custom-file-input"
                            id="questions_file" />
                        <label class="custom-file-label" for="questions_file">{{ __("Select csv file") }}</label>
                    </div>
                </div>
            </div>
        </div>
