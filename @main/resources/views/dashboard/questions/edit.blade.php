...
<div class="dashboard-edit flex justify-between items-center py-3">
    <h4 class="dashboard-edit-title mb-1">{{ __("Edit Question") }}</h4>
    <x-common.top-btn :icon="'fa-bars'" :text="__('All Questions')" class="mb-0" href="{{ route('dashboard.questions.list') }}" />
</div>
@php
$options = !empty($question->content_data) ? json_decode($question->content_data) : [];
@endphp

<form method="POST" action="{{ route('dashboard.questions.edit', $question->id) }}" enctype="multipart/form-data">
    @csrf
        ...
        <div class="lg:col-span-8 col-span-12">
            <div class="dashboard-edit p-10">
                <div class="eduman-select-field mb-5">
                    ...
                    <div class="eduman-input-field-style">
                        <div class="single-input-field w-full">
                            <x-text-input id="title" name="title" class="block mt-1 w-full" type="text" :value="$question->title" required autofocus />
                        </div>
                    </div>
                    @if ($errors->has("title"))
                ...
                <div class="item-container">
                    @forelse ($options->options ?? [] as $key => $item)
                ...
                <div class="eduman-select-field mb-5">
                    ...
                    <div class="eduman-input-field-style">
                        <div class="single-input-field w-full">
                            <x-text-input id="correct_answer" name="correct_answer" class="block mt-1 w-full" type="text" :value="$options->correct" required autofocus />
                        </div>
                    </div>
                ...
                <div class="eduman-select-field mb-5">
                    ...
                    <div class="eduman-input-field-style">
                        <div class="single-input-field w-full">
                            <x-text-input id="points" name="points" class="block mt-1 w-full" type="text" :value="$options->points ?? 0" required autofocus />
                        </div>
                    </div>
                ...
                <div class="eduman-select-field mb-5">
                    ...
                    <div class="eduman-input-field-style">
                        <div class="single-input-field w-full">
                            <textarea name="explanation" id="explanation" cols="30" rows="10">{{ $question->explanation }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="eduman-select-field-style">
                        <select class="block" id="category" name="category">
                            ...
                            @foreach ($types as $type)
                                <option @selected($question->category == $type->value) value="{{ $type->value }}">
                                    {{ $type->value }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="eduman-radio-field-style">
                        <div class="single-input-field w-full">
                            <label for="opt-1" class="mr-4">
                                <input class="mr-1" @checked($question->status->value == 'Active') type="radio" name="status" id="opt-1" value="Active" />
                                {{ __("Published") }}
                            </label>
                            <label for="opt-2" class="mr-4">
                                <input class="mr-1" @checked($question->status->value == 'Pending') type="radio" name="status" id="opt-2" value="Pending" />
                                {{ __("Pending") }}
                            </label>
                        </div>
                    </div>
