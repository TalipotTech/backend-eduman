...
@section('title') {{ __("Update Category") }} @endsection
...
@section('breadcrumb')
    @include('layouts.breadcrumb', ["path_array" => [
        ...
        __("Category") => route("dashboard.category.list"),
        __("Update Category") => ""
    ]])
@endsection
...
<div class="dashboard-edit flex justify-between items-center py-3">
    <h4 class="dashboard-edit-title mb-1">{{ __("Update Category") }}</h4>
    <x-common.top-btn :icon="'fa-bars'" :text="__('All Categories')" class="mb-0" href="{{ route('dashboard.category.list') }}" />
</div>

<form method="POST" action="{{ route('dashboard.category.edit', $category->id) }}" enctype="multipart/form-data">
    @csrf
            ...
            <div class="dashboard-edit p-10">
                <div class="eduman-select-field mb-5">
                    ...
                    <div class="eduman-input-field-style">
                        <div class="single-input-field w-full">
                            <x-text-input id="title" name="title" class="block mt-1 w-full" :value="$category->title" type="text" required autofocus />
                        </div>
                    </div>
                ...
                <div class="col-span-12">
                    <div class="eduman-select-field mb-5">
                        ...
                        <div class="eduman-input-field-style">
                            <div class="single-input-field w-full">
                                <x-tinymce.editor :id="'description'" :name="'description'" :type="'html'"  :category="$category->description" />
                            </div>
                        </div>
                    ...
                    <div class="eduman-radio-field-style">
                        <div class="single-input-field w-full">
                            <select class="block" id="ctype" name="ctype">
                                ...
                                <option {{ $category->type->value == 'Course' ? 'selected' : '' }} value="Course">{{ __("Course") }}</option>
                                <option {{ $category->type->value == 'Event' ? 'selected' : '' }} value="Event">{{ __("Event") }}</option>
                                <option {{ $category->type->value == 'Blog' ? 'selected' : '' }} value="Blog">{{ __("Blog") }}</option>
                            </select>
                        </div>
                    </div>
