...
@php
    $home_03_header_features = !empty($options["home_03_header_features"]) ? json_decode($options["home_03_header_features"]) : [];
@endphp
<form method="POST" action="{{ route('dashboard.settings.page.home-03.header') }}" enctype="multipart/form-data">
    @csrf
    <div class="grid grid-cols-12 sm:gap-x-5">
        <div class="lg:col-span-12 md:col-span-12 col-span-12">
            <div class="dashboard-edit p-10">
                @include("partials.inputs.input", [
                    "label" => __("home_03_header_sub_title"),
                    "name" => "home_03_header_sub_title",
                    "type" => "text",
                    "value" => $options["home_03_header_sub_title"] ?? "",
                ])
                @include("partials.inputs.input", [
                    "label" => __("home_03_header_title"),
                    "name" => "home_03_header_title",
                    "type" => "text",
                    "value" => $options["home_03_header_title"] ?? "",
                ])
               
                @include("partials.inputs.file", [
                    "label" => __("home_03_header_image"),
                    "name" => "home_03_header_image",
                    "value" => $options["home_03_header_image"] ?? "",
                ])

                @include("partials.inputs.input", [
                    "label" => __("home_03_header_button_text"),
                    "name" => "home_03_header_button_text",
                    "type" => "text",
                    "value" => $options["home_03_header_button_text"] ?? "",
                ])
                @include("partials.inputs.input", [
                    "label" => __("home_03_header_button_url"),
                    "name" => "home_03_header_button_url",
                    "type" => "text",
                    "value" => $options["home_03_header_button_url"] ?? "",
                ])
