...
@php
$page_title = __("Homepage-02 Features");
@endphp
...
@endsection
@php
$item_titles = !empty($options["home_02_feature_items_title"]) ? json_decode($options["home_02_feature_items_title"]) : [];
$item_images = !empty($options["home_02_feature_items_image"]) ? json_decode($options["home_02_feature_items_image"]) : [];
@endphp
@section('content')
...
<form method="POST" action="{{ route('dashboard.settings.page.home-02.features') }}" enctype="multipart/form-data">
    @csrf
                    ...
                    @forelse ($item_titles ?? [] as $key => $title)
                        <div class="repeater border-spacing-1 border-gray-300">
                            ...
                            @include("partials.inputs.input", [
                                "label" => __("home_02_feature_items_title"),
                                "name" => "home_02_feature_items_title[]",
                                "type" => "text",
                                "value" => $title ?? "",
                            ])
                            @include("partials.inputs.file", [
                                "label" => __("home_02_feature_items_image"),
                                "name" => "home_02_feature_items_image[]",
