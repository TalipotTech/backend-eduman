...
@section('content')
    <div class="dashboard-edit">
        <div class="mb-10">
            ...
            <x-common.top-btn :text="__('New Author')" href="{{ route('dashboard.courseAuthor.new') }}" />
        </div>
        <table id="datatable-org" class="table hover stripe table-auto" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $author->id }}</td>
                    <td>{{ $author->course->title }}</td>
                    <td>{{ $author->author->first_name }} {{ $author->author->last_name }}</td>
                    <td>{{ $author->author->email }}</td>
                    <td>{{ $author->author->phone}}</td>
                    <td>
                        <x-common.table.btn-dropdown :title=" __('Action') ">
                            <x-common.table.btn-dropdown-option :type="__('edit')" href="{{ route('dashboard.courseAuthor.edit', $author->id) }}" />
                            <x-common.table.btn-dropdown-option :type="__('delete')" class="delete-item" data-url="{{ route('dashboard.courseAuthor.delete', $author->id) }}" />
